...
"object points" were prepared to start with - the (x, y, z) coordinates of the chessboard corners in the world. Assumption: The chessboard is fixed on the (x, y) plane at z=0 (depth), such that the object points are the same for each calibration image.  
Thus, `objp` is just a replicated array of coordinates, and `objpoints` will be appended with a copy of it every time I successfully detect all chessboard corners in a test image.  `imgpoints` will be appended with the (x, y) pixel position of each of the corners in the image plane with each successful chessboard detection.  

The following OpenCV functions: **findChessboardCorners** and **drawChessboardCorners** were used to identify the locations of corners on chessboard photos in **camera_cal** directory.

Outputs `objpoints` and `imgpoints` were usedto compute the camera calibration and distortion coefficients using the `cv2.calibrateCamera()` function.  I then applied this distortion correction to the test image using the `cv2.undistort()` function and obtained this result:
...
Distortion correction was applied to raw images placed in directory **test_images**.
...
A combination of color and gradient thresholds was used to generate a binary image (thresholding steps in **11th** cell of [Advanced-Lane-Lines.ipynb](Advanced-Lane-Lines.ipynb).  
...
The code for perspective transform is in **9th code** cell in  [Advanced-Lane-Lines.ipynb](Advanced-Lane-Lines.ipynb).
This includes a function called `birds_eye_view()` which takes as inputs an image (`img`) and hardcodes the source (`src`) and destination (`dst`) points.  
...
In order to make it more robust, the binary channels selection needs to be revisited which could help me decide if there is any other combination that can work well with shadows and steep curves. 
